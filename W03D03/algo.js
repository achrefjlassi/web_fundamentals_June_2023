var pokémon = [
    ...
    { "id": 24,  "name": "Arksdjf",      "types": ["poison"] },
...
// Pokémon have more than one type
// for (var i=0;i<pokémon.length;i++) {
...
for (var i=0;i<pokémon.length;i++) {
    if(pokémon[i].types=="poison") {
        for(var j = 0 ; j<pokémon[i].name.length/2; j++){
            console.log(pokémon[i].name[j],'---', pokémon[i].name[pokémon[i].name.length -1 -j]);
            pokémon[i].name[j] = pokémon[i].name[pokémon[i].name.length -1 -j] // Reverse 
        }
    }
}
